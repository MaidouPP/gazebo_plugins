...
class TrajectoryActorPluginPrivate
{
  ...
  /// reached.
  public: double targetRadius{0.2};
...
{
  // google::SetLogDestination(0, "/home/users/lshixin/catkin_ws/src/walking_person_plugin/log/info.log");
  // google::InitGoogleLogging("gazebo_actor_log_info");
  gzerr << "================================ exo me???????"
        << std::endl;
  ...
  // even though target poses are Pose3d
  // what we need to control is only the Vector3d
  ...
  // LOG(INFO) << "Found " << this->numOfTrajs << " trajectories";

  // this->waypoints = this->trajs[0];
  int cnt = this->trajs[0].size();
  for (int j = 0; j < this->trajs[0].size(); j++) {
    ignition::math::Vector3d tmp(trajs[0][j][0], trajs[0][j][1], 1);
    this->dataPtr->targets.push_back(tmp);
  }
}
...
void TrajectoryActorPlugin::UpdateTarget()
{
  ...
  // Current target
  auto target = this->dataPtr->targets[this->dataPtr->currentTarget];
...
void TrajectoryActorPlugin::OnUpdate(const common::UpdateInfo &_info)
{
  ...
  // Direction to target
  auto dir = (targetPose - actorPose.Pos()).Normalize();
  ...
  if (std::abs(yawDiff.Radian()) > IGN_DTOR(10))
  {
    ...
    if (!this->dataPtr->cornerAnimation)
    {
      ...
      auto prevTargetPos = this->dataPtr->targets[previousTarget];
